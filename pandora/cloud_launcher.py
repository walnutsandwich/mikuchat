...
def main():
    ...
    try:
        from pandora.my_pandora_cloud.server import ChatBot as CloudServer
        print("导入my_pandora_cloud.server")
        # from pandora.bots.server import ChatBot as CloudServer

        return CloudServer(args.proxy, args.verbose, login_local=args.local).run(args.server, args.threads)
        # return CloudServer(args.proxy, args.verbose).run(args.server, args.threads)
